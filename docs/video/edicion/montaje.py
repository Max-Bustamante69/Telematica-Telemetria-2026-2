...
NORM, SRT, ASSETS, WORK, EXPORT = [os.path.join(W, d) for d in ("norm", "srt", "assets", "work", "export")]
...
Style: Sub,Segoe UI,46,&H00FFFFFF,&H00FFFFFF,&H00101010,&H96000000,-1,0,0,0,100,100,0,0,3,3,0,2,120,120,48,1
...
def write_ass(cues, path, offset=0.0):
    with open(path, "w", encoding="utf-8") as f:
        f.write(ASS_HEAD)
        for a, b, t in cues:
            f.write(f"Dialogue: 0,{ass_time(a+offset)},{ass_time(b+offset)},Sub,,0,0,0,,{two_lines(t)}\n")
...
def render_clip(c):
    ...
    if cut:
        ...
        vpre, apre = "[vc]", "[ac]"
    f.append(f"{vpre}format=yuv420p,split=2[base][cam]")
    ...
    if c["cam"] == "full" and c["overlays"]:
        ...
        f.append("[cam]scale=384:216,format=yuva420p[pip]")
    else:
        f.append("[cam]nullsink")
    for i, (ov, a, b) in enumerate(c["overlays"], 1):
        fade = f"fade=t=in:st={a}:d=0.4:alpha=1,fade=t=out:st={b-0.4}:d=0.4:alpha=1"
        if c["cam"] == "full":
    ...
    # tarjeta de título (0 a 5 s): banda oscura con título y nombre
    f.append(f"[{last}]drawbox=x=0:y=820:w=iw:h=260:color=black@0.72:t=fill:enable='between(t,0.3,5.3)',"
             f"drawtext=fontfile={FONTB}:text='{esc(c['title'])}':fontsize=64:fontcolor=white:x=120:y=865:enable='between(t,0.3,5.3)',"
             f"drawtext=fontfile={FONT}:text='{esc(c['who'])}':fontsize=40:fontcolor=0xCFCFCF:x=120:y=955:enable='between(t,0.3,5.3)',"
             f"drawtext=fontfile={FONT}:text='Plataforma distribuida de telemetría · Telemática · EAFIT':fontsize=30:fontcolor=0x9A9A9A:x=120:y=1010:enable='between(t,0.3,5.3)'[vt]")
    f.append(f"[vt]ass='{os.path.basename(ass)}'[vout]")
    out = os.path.join(WORK, c["id"] + ".mp4")
...
def main():
    ...
    final = os.path.join(EXPORT, "sustentacion-telemetria-1080p.mp4")
    ...
    write_srt(allcues, os.path.join(EXPORT, "sustentacion-telemetria.es-CO.srt"))
    vtt = os.path.join(EXPORT, "sustentacion-telemetria.es-CO.vtt")
    srt_txt = open(os.path.join(EXPORT, "sustentacion-telemetria.es-CO.srt"), encoding="utf-8").read()
