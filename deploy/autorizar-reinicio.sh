...
if [ "${1:-}" = "--revocar" ]; then
  ssh -i "$KEY" "ubuntu@$HOST" "sed -i '/ $2\$/d' ~/.ssh/authorized_keys && echo revocada: $2"
  exit 0
fi
PUB="$1"
LINE="command=\"/usr/local/bin/reiniciar-telemetria\",no-port-forwarding,no-X11-forwarding,no-agent-forwarding,no-pty $PUB"
ssh -i "$KEY" "ubuntu@$HOST" "grep -qF '$PUB' ~/.ssh/authorized_keys || echo '$LINE' >> ~/.ssh/authorized_keys; tail -1 ~/.ssh/authorized_keys | cut -c1-80"
